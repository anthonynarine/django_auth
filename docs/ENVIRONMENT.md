...
| `DEFAULT_FROM_EMAIL` | `settings.py` | From-address for all outgoing SendGrid mail |
| `SENDGRID_API_KEY` | `settings.py`, `views.py` | SendGrid API auth |
...
- Missing `SENDGRID_API_KEY` / `DEFAULT_FROM_EMAIL` → registration and password-reset emails fail silently (both are best-effort; the request itself still succeeds).
